...
class SettingsController extends Controller
{
    ...
    //        echo '</pre>';
    //        exit;
            /*$Sql = 'SELECT * FROM lib_hospcode WHERE off_id="'.$model->hcode.'" ';
            $hospInfo = Yii::$app->db_dc->createCommand($Sql)->queryOne();

            $model->prov = $hospInfo["changwat"]==""? $model->prov:$hospInfo["changwat"];

            $Sql = 'SELECT off_id,name '.
                'FROM lib_hospcode WHERE changwat="'.$model->prov.
                '" AND typecode NOT IN ("01","02","03","04","16","17") ORDER BY typecode,name';
            $rawData = Yii::$app->db_dc->createCommand($Sql)->queryAll();
            $hospName = [];
            foreach ($rawData as $value) {
                $hospName[$value["off_id"]] = $value["name"];
            }*/

            $Sql = 'SELECT code,position_type FROM lib_position_type ORDER BY code';
            $rawData = Yii::$app->db_dc->createCommand($Sql)->queryAll();
            $posType = [];
            foreach ($rawData as $value) {
                $posType[$value["code"]] = $value["position_type"];
            }
        }
